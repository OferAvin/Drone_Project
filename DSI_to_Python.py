...
class TCPParser: # The script contains one main class which handles DSI-Streamer data packet parsing.
	...
	def example_plot(self,table):
		...
		while True: # runtime < duration/refresh_rate:
			...
			feature[0, 4:6] = get_bandpower(Pxx_den, target_frq, f, stride)[0:1]
			feature[0, 6:] = get_bandpower(Pxx_den, harm_frq[0], f, stride)[0:1]
			#Predict
			main_power = np.mean(feature[0, 0:2])
			if main_power > 15:
				y_pred = 3
			else:
				y_pred = 2
			...
			# plt.clf()
			# try:
			# 	plt.plot(f[6:52], np.transpose(Pxx_den[:, 6:52]))
			# except: pass
